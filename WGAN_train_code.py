...
def gradient_penalty(discriminator,batch_x,fake_image):
    ...
    interplate = tf.minimum(tf.minimum((0.5+Alpha),1.0)*(t*batch_x+(1-t)*fake_image),
                            (tf.minimum((0.5+Beta),1.0) * (t*batch_x+(1-t)*fake_image)))
    min_term = interplate
    with tf.GradientTape() as tape:
        tape.watch([min_term])
        #D(X)
        d_interplote_logits = discriminator(interplate)
        max_term = tf.minimum(tf.minimum((0.5 + tf.abs(Alpha_)),1.0) * d_interplote_logits,
                              tf.minimum((0.5 + tf.abs(Beta_)),1.0) * d_interplote_logits)
...
def train():
    ...
    plt.ylabel("Loss")
    plt.legend(loc="best")
    plt.savefig('./plt.save/G loss.png')
    plt.show()
    ...
    plt.ylabel("Loss")
    plt.legend(loc="best")
    plt.savefig('./plt.save/D loss.png')
    plt.show()

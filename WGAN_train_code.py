...
def d_loss_fn(generator,discriminator,batch_z,batch_x,is_training):
    ...
    prev_batch_x = tf.Variable(initial_value=tf.zeros(shape=(batch_size, 100, 100, 1)))
    prev_fake_image = tf.Variable(initial_value=tf.zeros(shape=(batch_size, 100, 100, 1)))
    Alpha = prev_fake_image / (prev_fake_image + fake_image)
    Beta = prev_batch_x / (prev_batch_x + batch_x)
    gp = gradient_penalty(discriminator, tf.minimum((0.5+Alpha)*batch_x, (0.5+Beta)*batch_x , batch_x), tf.maximum((0.5+Alpha)*fake_image, (0.5+Beta)*fake_image, fake_image))
